...
#define LEAF_GADGET protoboard<FieldT_A> pb; \
    ...
    \
        ...
        merkle_tree_check_update_gadget<FieldT_A, HashT_A> mls(pb, tree_depth, address_bits_va, \
                                                               ...
                                                               next_leaf_digest, next_root_digest, next_path_var, pb_variable<FieldT_A>(1), "mls"); \
...
int main(void)
{
        ...
        for (int i = 0; i < test_num; i++) {
                //clock_t Begin = clock();
                double duration = test_leaf_gen< libff::mnt4_pp, CRH_with_bit_out_gadget<libff::Fr<libff::mnt4_pp> > >("mnt4");
                //clock_t End = clock();
        ...
        for (int i = 0; i < test_num; i++) {
                /*
                      ...
                      double duration = double(End - Begin) / CLK_TCK;
                 */
                double duration = test_leaf_example<libff::mnt4_pp, FieldT_A, CRH_with_bit_out_gadget<FieldT_A> >(pk, "mnt4");
                fprintf(file2, "%lf\n", duration);
                tot += duration;
        }

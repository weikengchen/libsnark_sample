...
#define LAYER2_GADGET(leaf_vk) protoboard<FieldT_B> pb; \
        ...
        primary_input_2_bits_first_half.insert(primary_input_2_bits_first_half.end(), primary_input_2_bits.begin() + 1 - 1, primary_input_2_bits.begin() + 297 - 1 + 1); \
        primary_input_2_bits_first_half.insert(primary_input_2_bits_first_half.end(), primary_input_2_bits.begin() + 299 - 1, primary_input_2_bits.begin() + 299 - 1 + 1); \
...
template<typename ppT_A, typename ppT_B> double test_layer2_prove(r1cs_ppzksnark_proving_key<ppT_B> &pk, const std::string &annotation) {
        ...
        primary_input_1_first_hash.insert(primary_input_1_first_hash.end(), primary_input_1_as_bits.begin() + 1 - 1, primary_input_1_as_bits.begin() + 297 - 1 + 1);
...
int main(void)
{
        ...
        for (int i = 0; i < test_num; i++) {
                //clock_t Begin = clock();
                double duration = test_layer2_gen< libff::mnt4_pp, libff::mnt6_pp >("mnt4->6");
                //clock_t End = clock();
        ...
        for (int i = 0; i < test_num; i++) {
                //clock_t Begin = clock();
                double duration = test_layer2_prove< libff::mnt4_pp, libff::mnt6_pp >(pk, "mnt4->6");
                //clock_t End = clock();
                //double duration = double(End - Begin) / CLK_TCK;
                fprintf(file2, "%lf\n", duration);
                tot += duration;
